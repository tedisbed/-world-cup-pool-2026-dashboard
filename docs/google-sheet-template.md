...
Use one Google Sheet tab as the published CSV source for the dashboard state.
The first row must be headers. Every data row must have a `type` and an `id`;
rows missing either value are ignored.
...
1. Create a Google Sheet with one tab containing the header and rows above.
...
6. Put that URL in `data/source-config.json`:
...
- The Sheet source is state input, not the same as the dashboard's exported
  `Matches CSV` or `Scores CSV`.
...
- Configured sources are tried first. If the Sheet URL is blank, private,
  unpublished, or unavailable, the app falls back to `data/state.json`, then to
  browser local storage, then to an empty pool state.
- Google published CSV updates can be cached by Google or the browser. After
  editing the Sheet, allow a short delay and hard-refresh the GitHub Pages page
